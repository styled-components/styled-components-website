...
CSS Compatibility matrix: `utils/cssCompat.ts` is the hand-authored compatibility table backing `/docs/compatibility` (the "React Native CanIUse"). Columns: SC v6, SC v7, stock RN iOS, stock RN Android. caniuse-lite browser minimums are joined in `utils/cssCompat.server.ts` (server-only, kept out of the client bundle). Page lives at `app/docs/compatibility/page.tsx`, rendered by the client component `components/CompatibilityMatrix.tsx` (dense grid, click row to expand detail, sticky header). When changing styled-components CSS support, update `cssCompat.ts` in the same pass.

Matrix entry rules:
- Feature titles must be CSS spec names only (e.g., `vertical-align`, `text-decoration-style`). RN-equivalent style keys (e.g., `textAlignVertical`, `shadowColor`) and value enumerations belong in the entry's `summary` field, not the title.
- Only include features that are CSS-relevant on React Native, or that could plausibly become CSS on RN. Pure web concerns (vendor prefixes, RSC-specific selector edge cases, etc.) do not belong here.
- The v6 / v7 columns describe SC's combined-platform native behavior. If iOS stock = yes but Android stock = no, v7 (which passes through to stock) should be `partial`, not `no`. Give v7 credit when at least one platform works through the runtime.
- `prUrls` is keyed per column (`v6`, `v7`, `ios`, `android`). Set entries only on the columns whose status is `no` and an upstream PR exists; the cell renders the hourglass icon linking to that PR.
...
- Palette generation: seed at target L/C/H then `qlab separate --adaptive --tolerance tight --gamut p3` then `qlab harmonize --adaptive --tolerance tight --gamut p3`. Both passes, in order. Read palette comments in `theme.ts` for current ΔE and seed values.
