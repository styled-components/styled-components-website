...
function injectStartScreen(modal: Element, latestPost: LatestPost) {
...
function initDocSearch(container: HTMLElement, latestPost: LatestPost, onReady: () => void) {
  ...
  import('@docsearch/js').then(mdl => {
    mdl.default({
      ...
      // `searchInputAriaLabel` is present in @docsearch/react's runtime but
      // missing from its TS types; cast through Record to narrow the gap
...
export default function AlgoliaSearch({
  latestPost,
  ...props
}: React.HTMLAttributes<HTMLDivElement> & { latestPost: LatestPost }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const latestPostRef = useRef(latestPost);
  const [ready, setReady] = useState(docSearchInitialized);

  useEffect(() => {
    if (!containerRef.current || process.env.NODE_ENV === 'test') return;
    initDocSearch(containerRef.current, latestPostRef.current, () => setReady(true));
  }, []);
