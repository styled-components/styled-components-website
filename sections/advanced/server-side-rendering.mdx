...
With styled-components v6.3.0+, RSC is natively supported with zero configuration. Styled components work in Server Components without a `'use client'` directive or registry setup. If you use `:first-child` or `:nth-child()` selectors, add `stylisPluginRSC` (v6.4+) via `StyleSheetManager` in your layout (see [child-index selector caveat](#child-index-selector-caveat) above). See the [React Server Components](#react-server-components) section above for best practices.
