...
styled-components now natively supports React Server Components (RSC) with zero configuration.
...
In v7, import `rscPlugin` from the plugins subpath and pass it through the
renamed `plugins` prop:

```tsx
import { StyleSheetManager } from 'styled-components';
import { rscPlugin } from 'styled-components/plugins';

<StyleSheetManager plugins={[rscPlugin]}>{children}</StyleSheetManager>
```
...
Render those elements without adding React's `precedence` or `href` props.
React-managed precedence tags lose the styled-components attributes required
for client adoption, so each server rule would otherwise be injected again
during hydration.
...
With styled-components v6.3.0+, styled components defined in Server Components work without a `'use client'` directive or registry setup. A registry is still needed to collect styles from Client Components during SSR. If you use `:first-child` or `:nth-child()` selectors, add `stylisPluginRSC` (v6.4) or `rscPlugin` (v7) via `StyleSheetManager` in your layout (see [child-index selector caveat](#child-index-selector-caveat) above). See the [React Server Components](#react-server-components) section above for best practices.
