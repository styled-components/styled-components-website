...
- **`attr(name <type>?, fallback?)`** reads the component's own props as typed CSS values and composes inside `calc()`.
- **`sibling-index()` and `sibling-count()`** resolve an element's 1-indexed position and sibling total inside a styled parent, updating when children mount, unmount, or reorder.
...
Webpack, Vite, and Metro web targets automatically select v7's dedicated
`react-native-web` build from the normal `styled-components/native` import. If
your bundler does not select it, import
