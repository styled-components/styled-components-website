...
This behavior is only supported within the context of _Styled_ Components: interpolating `A` in the following example will trigger a development warning because component `A` is not a Styled Component.
...
In development, a `console.error` warning is emitted because `A` is a plain function component being used as an interpolation. The component still mounts, but the selector won't work as intended.
