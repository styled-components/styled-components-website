...
const DuplicatedStyledComponents = () => md`
  ...
  If you identified that duplication is the issue that you are encountering there are several thing you
  can try to solve it:

  If you are using \`npm\` you can try running \`npm dedupe\`. This command should searches the local dependencies and
  ...
  If you are using webpack, you can change the way how webpack will [resolve](https://webpack.js.org/configuration/resolve/#resolve-modules)
  \`styled-components\` module. You can overwrite the default order in which webpack will look for your dependencies 
  and make the app \`node_modules\` more prioritized than default node module resolution order:
