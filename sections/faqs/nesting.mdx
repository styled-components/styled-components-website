...
> **RSC caveat:** In React Server Components, inline `<style>` tags are emitted as siblings of your elements, which shifts `:first-child` and `:nth-child()` indices. Use `:first-of-type` / `:nth-of-type()` instead, or enable [`stylisPluginRSC`](/docs/api#stylesheetmanager) to rewrite these selectors automatically.
