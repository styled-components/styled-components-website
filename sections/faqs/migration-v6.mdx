...
Styled components now work in RSC environments without `'use client'` directives. `ThemeProvider` and `StyleSheetManager` become no-ops in RSC. Use CSS custom properties for theming instead of `ThemeProvider` in server components. See the [RSC section](/docs/advanced#react-server-components) for best practices.
