...
> It's highly recommended (but not required) to also use the [Babel plugin](/docs/tooling#babel-plugin) or [SWC plugin](/docs/tooling#swc-plugin). They offer many benefits like more legible class names, server-side rendering compatibility, smaller bundles, and more.
...
> This style of usage requires the React CDN bundles (`react` and `react-dom`) to be on the page as well (before the styled-components script.) See [React's CDN documentation](https://react.dev/learn/add-react-to-an-existing-project#using-react-for-a-part-of-your-existing-page) for details.
