...
### Polymorphic `as` targets | v7

The `as` prop changes both the rendered element and the attributes TypeScript
offers for it. For example, after writing `<Media as="video"`, your editor
completes video attributes such as `loop`, `muted`, `controls`, and `poster`.
Without `as`, completion stays on the styled component's own props.
...
<Media as="video" controls muted />;
```

This improves editor completion only; the runtime behavior of `as` is
unchanged.

When styling a custom React component, styled-components passes all custom props:
