...
> React Server Components (v6.3.0+): `StyleSheetManager` becomes a pass-through component (no-op) in RSC environments. CSS is automatically delivered via inline `<style>` tags. See the [RSC section](/docs/advanced#react-server-components) for details.
